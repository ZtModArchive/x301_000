...
sampler ScumSamp = sampler_state
{
MinFilter = LINEAR;
MagFilter = LINEAR;
MipFilter = LINEAR;
...
void CalcScumColors(const in float3 norm,
					const in float3 liDir,
					const in float4 ambient,
					const in float4 diffuse,
					const in float opacFilter,
					out float4 modColor)
{
	float nDOTdir = saturate(-dot(norm, liDir));
	
	modColor.rgb = ambient.rgb + diffuse.rgb * nDOTdir;
	modColor.a = diffuse.a * opacFilter;
}
...
void CalcEyeRay(const in float4 wPos, 
								  const in float4 cameraPos, 
								  out float3 cam2Vtx)
{
	// Get normalized vec from camera to vertex, saving original distance.
	cam2Vtx = float3(wPos.xyz) - float3(cameraPos.xyz);
	float pertAtten = length(cam2Vtx);
	cam2Vtx = cam2Vtx / pertAtten;
}
...
void CalcScumCoords(const in float2 wPos, 
				const in float4 kScumOffsetLo,
				const in float4 kScumOffsetHi,
				out float2 coord0,
				out float2 coord1)
{
	coord0 = wPos.xy * kScumOffsetLo.xy + kScumOffsetLo.zw;
	coord1 = wPos.xy * kScumOffsetHi.xy + kScumOffsetHi.zw;
}
...
ScumOut vs_scum(VertIn In,
				uniform float4x4 kWorld2NDC,
				uniform float4 kLightDir,
				uniform float4 kAmbient,
				uniform float4 kDiffuse,
				uniform float4 kScumOffsetLo,
				uniform float4 kScumOffsetHi,
				uniform float4 kFrequency,
				uniform float4 kPhase,
				uniform float4 kAmplitude,
				uniform float4 kPosX,
...
//FillMode=WIREFRAME;
	...
	pass P1
	{
		VertexShader = compile vs_1_1 vs_scum(cWorld2NDC,
											cScumLightDir,
											cScumAmbient,
											cScumDiffuse,
											cScumOffsetLo,
